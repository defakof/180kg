...
#pragma region variables_legitbot
	...
	// --- NEW Autofire Variables ---
	C_ADD_VARIABLE(bool, bEnableAutofire, false); // Renamed autofireEnemy for clarity
	C_ADD_VARIABLE(bool, bEnableAutostop, false); // Renamed autostopforfire for clarity
	// Optional: Add keybind if you want autofire only when a key is held
	// C_ADD_VARIABLE(unsigned int, nAutofireKey, 0); // 0 means always on when enabled
#pragma endregion
};
