...
void MENU::RenderMainWindow()
{
	...
	if (ImGuiWindow* pMainWindow = ImGui::FindWindowByName(CS_XOR("180kg")); pMainWindow != nullptr)
	{
	...
	// render main window
	ImGui::Begin(CS_XOR("180kg"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse);
	{
		...
		const ImVec2 vecTitleSize = FONT::pMenu[C_GET(int, Vars.nDpiScale)]->CalcTextSizeA(12.f * flDpiScale, FLT_MAX, -1.f, CS_XOR("180kg"));
		pDrawList->AddText(ImVec2(vecMenuPos.x + vecMenuSize.x - style.WindowPadding.x - vecTitleSize.x, vecMenuPos.y + style.WindowPadding.y), ImGui::GetColorU32(ImGuiCol_Text), CS_XOR("180kg"));
...
void MENU::RenderOverlayPreviewWindow()
{
	...
	ImGui::Begin(CS_XOR("preview"), &bMainWindowOpened, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
	{
		...
		if (const auto& nameOverlayConfig = C_GET(TextOverlayVar_t, Vars.overlayName); nameOverlayConfig.bEnable)
			context.AddComponent(new CTextComponent(true, SIDE_TOP, DIR_TOP, FONT::pVisual, CS_XOR("180kg"), Vars.overlayName));
...
void MENU::RenderWatermark()
{
	...
	ImGui::BeginMainMenuBar();
	{
		...
		static ImVec2 vecNameSize = ImGui::CalcTextSize(CS_XOR("180kg | " __DATE__ " " __TIME__));
		ImGui::SameLine(ImGui::GetContentRegionMax().x - vecNameSize.x - style.FramePadding.x);
		ImGui::TextColored(ImVec4(1.0f, 1.0f, 1.0f, 1.0f), CS_XOR("180kg | " __DATE__ " " __TIME__));
	}
...
void T::LegitBot()
{
	...
	// --- NEW Autofire Child Window ---
	ImGui::BeginChild(CS_XOR("legitbot.autofire"), ImVec2{}, true, ImGuiWindowFlags_MenuBar);
	{
		if (ImGui::BeginMenuBar())
		{
			ImGui::TextUnformatted(CS_XOR("autofire"));
			ImGui::EndMenuBar();
		}

		ImGui::Checkbox(CS_XOR("enable##autofire"), &C_GET(bool, Vars.bEnableAutofire));
		ImGui::Checkbox(CS_XOR("autostop when firing##autofire"), &C_GET(bool, Vars.bEnableAutostop));

		// Optional: Add keybind control if you added the variable
		// ImGui::HotKey(CS_XOR("activation key##autofire"), &C_GET(unsigned int, Vars.nAutofireKey));
	}
	ImGui::EndChild();
}
