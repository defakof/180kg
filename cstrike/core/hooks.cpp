...
bool CS_FASTCALL H::CreateMove(CCSGOInput* pInput, int nSlot, CUserCmd* cmd)
{
	...
	if (SDK::LocalPawn == nullptr || SDK::LocalPawn->GetLifeState() != LIFE_ALIVE) // Ensure pawn is valid and alive
		return bResult;

	// --- Call existing feature function (like aimbot, BHop, etc.) ---
// Example: F::OnCreateMove might handle Aimbot angle changes
	F::OnCreateMove(SDK::Cmd, pBaseCmd, SDK::LocalController);
	// IMPORTANT: The aimbot logic inside F::OnCreateMove should have already modified
	// pBaseCmd->pViewAngles if necessary. Autofire will use these potentially modified angles.

	// --- START OF AUTOFIRING LOGIC ---
	if (C_GET(bool, Vars.bEnableAutofire))
	{
		// Optional: Check for activation key if you added one
		// if (C_GET(unsigned int, Vars.nAutofireKey) != 0 && !IPT::IsKeyDown(C_GET(unsigned int, Vars.nAutofireKey)))
		// {
		//     // Key not held, don't autofire
		// }
		// else
		{
			// Get the active weapon
			CBaseHandle hWeapon = SDK::LocalPawn->GetWeaponServices()->GetActiveWeapon();
			C_CSWeaponBaseGun* pWeapon = I::GameResourceService->pGameEntitySystem->Get<C_CSWeaponBaseGun>(hWeapon);

			// Check if weapon is valid, has ammo, isn't reloading, etc.
			if (pWeapon != nullptr && pWeapon->GetClip1() > 0 && !pWeapon->IsInReload() /* Add CanPrimaryAttack check if needed */)
			{
				// Get current view angles (potentially modified by aimbot)
				QAngle_t angView = pBaseCmd->pViewAngles->angValue;

				// Perform a TraceRay to see what's under the crosshair
				Vector vecEyePos = SDK::LocalPawn->GetEyePosition();
				Vector vecForward;
				Math::AngleVectors(angView, &vecForward); // Use your math library
				Vector vecEnd = vecEyePos + vecForward * 8192.f; // Max trace distance

				GameTrace_t tr;
				Ray_t ray; // May need initialization depending on TraceShape signature
				// Use appropriate mask and skip local player
				TraceFilter_t filter(CONTENTS_SOLID | CONTENTS_GRATE | CONTENTS_PLAYERCLIP | CONTENTS_MONSTER, SDK::LocalPawn, nullptr, 4); // Example mask/layer, adjust as needed

				I::GameTraceManager->TraceShape(&ray, vecEyePos, vecEnd, &filter, &tr);

				bool bShouldFire = false;
				if (tr.m_pHitEntity != nullptr)
				{
					// Check if the hit entity is a player pawn
					if (tr.m_pHitEntity->IsCSPlayerPawn()) // You might need to add an IsCSPlayerPawn() method or check schema name
					{
						C_CSPlayerPawn* pHitPawn = static_cast<C_CSPlayerPawn*>(tr.m_pHitEntity);
						// Check if the hit player is an enemy and alive
						if (pHitPawn->GetHealth() > 0 && SDK::LocalPawn->IsOtherEnemy(pHitPawn))
						{
							bShouldFire = true;
						}
					}
				...
				// Check if we should actually fire based on the trace result
				if (bShouldFire)
				{
					// Set the attack button state
					cmd->nButtons.nValue |= IN_ATTACK;

					// Apply autostop if enabled
					if (C_GET(bool, Vars.bEnableAutostop))
					{
	...
	// --- END OF AUTOFIRING LOGIC ---

	// --- CRC Handling ---
	// Ensure this runs *after* all modifications to pBaseCmd (including buttons, movement, angles)
	// CRC::Save(pBaseCmd);
	// if (CRC::CalculateCRC(pBaseCmd) == true)
	// 	CRC::Apply(SDK::Cmd);

	return bResult; // Crucial: Return the original result unless you intentionally want to block movement/angles
}
